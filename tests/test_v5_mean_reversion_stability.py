...
def gate(*, net: str = "0.01", pf: str = "1.01", positive: int = 7):
    return evaluate_v5_h0_gate(
        metrics=V5H0Metrics(Decimal(net), Decimal(pf)),
...
def test_all_preregistered_v5_h0_gates_pass() -> None:
...
def test_incorrect_eligible_window_count_is_rejected() -> None:
    with pytest.raises(ValueError, match="exactly 11"):
        evaluate_v5_h0_gate(
            metrics=V5H0Metrics(Decimal("0.01"), Decimal("1.01")),
