...
def build_h5_exit_trade_records(
    *,
    ...
    if not (len(trades) == len(r_records) == len(diagnostics)):
        raise ValueError("H5 diagnostic trade inputs are not aligned.")
    timestamp_index = {
    ...
    for trade, r_record, diagnostic in zip(
        trades, r_records, diagnostics, strict=True
    ):
        if not (
            trade.trade_id == r_record.trade_id == diagnostic.trade_id
            ...
            raise ValueError(f"Incomplete H5 diagnostics for {trade.trade_id}.")
        thresholds = calculate_excursion_thresholds(
            trade,
            dataset,
            timestamp_index,
            initial_risk_per_unit=diagnostic.initial_risk_per_unit,
        )
        output.append(
            H5ExitTradeRecord(
                ...
                first_four_bar_mae_r=diagnostic.first_four_bar_mae_r,
                holding_bars=diagnostic.bars_held,
                entry_atr_percent=diagnostic.entry_atr_percent,
                entry_rsi=diagnostic.entry_rsi,
                entry_volume_ratio=diagnostic.entry_volume_ratio,
                entry_ema_spread_percent=diagnostic.entry_ema_spread_percent,
                reached_positive_half_r=thresholds.reached_positive_half_r,
                reached_positive_one_r=thresholds.reached_positive_one_r,
                reached_positive_one_and_half_r=(
                    thresholds.reached_positive_one_and_half_r
                ),
                reached_positive_two_r=thresholds.reached_positive_two_r,
                reached_negative_half_r=thresholds.reached_negative_half_r,
                reached_negative_one_r=thresholds.reached_negative_one_r,
                bars_to_positive_half_r=thresholds.bars_to_positive_half_r,
                bars_to_positive_one_r=thresholds.bars_to_positive_one_r,
            )
        )
    return tuple(output)
...
def summarize_exit_groups(
    ...
    for reason in ExitReason:
        ...
        if not selected:
            continue
        winners = tuple(record.net_r for record in selected if record.net_r > 0)
        losers = tuple(record.net_r for record in selected if record.net_r < 0)
        ...
        rows.append(
            ExitGroupSummary(
                exit_reason=reason,
                trades=len(selected),
                percentage_of_all_trades=_percent(len(selected), len(records)),
                total_frictionless_r=sum(
                    (record.frictionless_r for record in selected), Decimal("0")
                ),
                total_net_r=sum(
                    (record.net_r for record in selected), Decimal("0")
                ),
                frictionless_expectancy_r=_mean(
                    record.frictionless_r for record in selected
                ),
                net_expectancy_r=_mean(record.net_r for record in selected),
                average_fee_r=_mean(record.fee_r for record in selected),
                average_slippage_r=_mean(record.slippage_r for record in selected),
                average_total_friction_r=_mean(
                    record.total_friction_r for record in selected
                ),
                win_rate_percent=_percent(len(winners), len(selected)),
                ...
                median_mfe_r=median(record.mfe_r for record in selected),
                median_mae_r=median(record.mae_r for record in selected),
                average_holding_bars=_mean(
                    Decimal(record.holding_bars) for record in selected
                ),
                median_holding_bars=median(
                    Decimal(record.holding_bars) for record in selected
                ),
                average_entry_atr_percent=_optional_mean(
                    record.entry_atr_percent for record in selected
                ),
                average_entry_rsi=_optional_mean(
                    record.entry_rsi for record in selected
                ),
                average_entry_volume_ratio=_optional_mean(
                    record.entry_volume_ratio for record in selected
                ),
                average_entry_ema_spread_percent=_optional_mean(
                    record.entry_ema_spread_percent for record in selected
                ),
            )
        )
    return tuple(rows)
...
def summarize_excursion_groups(
    ...
    groups = [
        ("ALL", records),
        ("LOSING_TRADES", tuple(record for record in records if record.net_r < 0)),
        *[
            (
                reason.value,
                tuple(record for record in records if record.exit_reason is reason),
            )
            for reason in ExitReason
            if any(record.exit_reason is reason for record in records)
        ],
    ]
    rows = []
        ...
        rows.append(
            ExcursionGroupSummary(
                ...
                reached_positive_half_r_percent=_percent(
                    sum(record.reached_positive_half_r for record in selected), count
                ),
                reached_positive_one_r_percent=_percent(
                    sum(record.reached_positive_one_r for record in selected), count
                ),
                ...
                reached_positive_two_r_percent=_percent(
                    sum(record.reached_positive_two_r for record in selected), count
                ),
                reached_negative_half_r_percent=_percent(
                    sum(record.reached_negative_half_r for record in selected), count
                ),
                reached_negative_one_r_percent=_percent(
                    sum(record.reached_negative_one_r for record in selected), count
                ),
                median_bars_to_positive_half_r=(
                    median(
                        Decimal(record.bars_to_positive_half_r)
                        for record in selected
                        if record.bars_to_positive_half_r is not None
                    )
                    if any(
                        record.bars_to_positive_half_r is not None
                        for record in selected
                    )
                    else None
                ),
                median_bars_to_positive_one_r=(
                    median(
                        Decimal(record.bars_to_positive_one_r)
                        for record in selected
                        if record.bars_to_positive_one_r is not None
                    )
                    if any(
                        record.bars_to_positive_one_r is not None
                        for record in selected
                    )
                    else None
                ),
                first_four_bar_negative_half_r_percent=_percent(
                    sum(
                        record.first_four_bar_mae_r >= Decimal("0.5")
                        for record in selected
                    ),
                    count,
                ),
