"""Frozen V8-H0 support and progression gates."""
...
V6_GROSS_EXPECTANCY_R = Decimal("0.04722009245553381286395292467")
...
class V8H0Metrics:
    ...
    non_v6_entry_count: int


@dataclass(frozen=True, slots=True)
class V8H0WindowMetrics:
    trades: int
    net_result_r: Decimal | None
    v6_net_result_r: Decimal

    def __post_init__(self) -> None:
        if self.trades < 0:
            raise ValueError("V8-H0 window trade count cannot be negative.")
        if self.trades > 0 and self.net_result_r is None:
            raise ValueError("A traded V8-H0 window requires a net result R.")

    @property
    def positive_net(self) -> bool:
        return (
            self.trades > 0
            and self.net_result_r is not None
            and self.net_result_r > 0
        )

    @property
    def net_result_better_than_v6(self) -> bool:
        return (
            self.trades > 0
            and self.net_result_r is not None
            and self.net_result_r > self.v6_net_result_r
        )


@dataclass(frozen=True, slots=True)
class V8H0SupportGateResult:
    eligible_windows: int
    windows_net_result_better_than_v6: int
    gross_expectancy_better: bool
...
@dataclass(frozen=True, slots=True)
class V8H0ProgressionGateResult:
    support: V8H0SupportGateResult
...
def _validate_windows(windows: tuple[V8H0WindowMetrics, ...]) -> None:
    if len(windows) != REQUIRED_ELIGIBLE_WINDOWS:
        raise ValueError(
            "V8-H0 requires exactly 11 eligible windows, got "
            f"{len(windows)}."
        )


def evaluate_v8_h0_support_gate(
    *,
    metrics: V8H0Metrics,
    windows: tuple[V8H0WindowMetrics, ...],
) -> V8H0SupportGateResult:
    """Apply only the preregistered V8-H0 support conditions."""

    _validate_windows(windows)
    net_better_windows = sum(
        window.net_result_better_than_v6 for window in windows
    )
    return V8H0SupportGateResult(
        eligible_windows=REQUIRED_ELIGIBLE_WINDOWS,
        windows_net_result_better_than_v6=net_better_windows,
        gross_expectancy_better=(
            metrics.gross_expectancy_r > V6_GROSS_EXPECTANCY_R
        ),
...
def evaluate_v8_h0_progression_gate(
    *,
    metrics: V8H0Metrics,
    windows: tuple[V8H0WindowMetrics, ...],
) -> V8H0ProgressionGateResult:
    """Apply the frozen progression gate after the support gate."""

    support = evaluate_v8_h0_support_gate(metrics=metrics, windows=windows)
    positive_windows = sum(window.positive_net for window in windows)
    return V8H0ProgressionGateResult(
        support=support,
